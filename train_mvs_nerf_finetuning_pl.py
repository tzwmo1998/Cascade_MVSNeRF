...
class MVSSystem(LightningModule):
    def __init__(self, args):
        ...
        if not self.args.use_casmvs:
            ...
            self.render_kwargs_train, self.render_kwargs_test, start, self.grad_vars = create_casnerf_mvs(args, use_casmvs=True, dir_embedder=False, pts_embedder=True)
    ...
    def init_volume(self):
        ...
        if 'volume' not in ckpts.keys():
            self.MVSNet.train()
            with torch.no_grad():
                volume_feature, _, depth_range = self.MVSNet(self.imgs, self.proj_mats, [self.near_far_source], pad=args.pad, lindisp=args.use_disp)
                if self.args.use_casmvs:
                    self.depth_range = depth_range
                else:
                    self.depth_range = None
                self.N_views = len(volume_feature)
        else:
            volume_feature = ckpts['volume']['feat_volume'].unsqueeze(0)
            self.N_views = 1
            if self.args.use_casmvs:
                self.depth_range = ckpts['depth_range']
    ...
    def training_step(self, batch, batch_nb):
        ...
        rgbs, disp, acc, depth_pred, alpha, extras = rendering(args, self.pose_source, xyz_coarse_sampled, xyz_NDC, z_vals, rays_o, rays_d,
                                                       self.volume, self.imgs,  depth_range=self.depth_range,**self.render_kwargs_train)
        ...
        with torch.no_grad():
            ...
            for chunk_idx in range(N_rays_all//args.chunk + int(N_rays_all%args.chunk>0)):
                ...
                rgb, disp, acc, depth_pred, alpha, extras = rendering(args, self.pose_source, xyz_coarse_sampled,
                                                                       ...
                                                                       self.volume, self.imgs, depth_range=self.depth_range,
...
#             depth_range = nn.Parameter(self.depth_range)
            ckpt['depth_range'] = self.depth_range
