...
class MVSSystem(LightningModule):
    ...
    def training_step(self, batch, batch_nb):
        ...
        volume_features, img_feat, depth_range = self.MVSNet(imgs[:, :3], proj_mats[:, :3], near_fars[0, :3],pad=args.pad)
    ...
    def validation_step(self, batch, batch_nb):
        ...
        with torch.no_grad():
            ...
            volume_features, img_feat, depth_range = self.MVSNet(imgs[:, :3], proj_mats[:, :3], near_fars[:3], pad=args.pad)
            if not self.args.use_casmvs:
                depth_range=None
            imgs = self.unpreprocess(imgs)
            rgbs, depth_preds = [],[]
            for chunk_idx in range(H*W//args.chunk + int(H*W%args.chunk>0)):
                ...
                # rendering
                rgb, disp, acc, depth_pred, density_ray, ret = rendering(args, pose_ref, rays_pts, rays_NDC, depth_candidates, rays_o, rays_dir, volume_features, imgs[:, :-1], img_feat=None, depth_range=depth_range, **self.render_kwargs_train)
