...
def gen_pts_feats(imgs, volume_feature, rays_pts, pose_ref, rays_ndc, feat_dim, img_feat=None, img_downscale=1.0, use_color_volume=False, net_type='v0', depth_range=None, pad=0):
    shape = imgs.shape
    N_rays, N_samples = rays_pts.shape[:2]
    N_views = len(volume_feature)
    ...
    if not use_color_volume:
        ...
        if torch.is_tensor(volume_feature[0]):
            ray_feats = index_point_feature(shape, volume_feature, rays_pts, pose_ref, depth_range=depth_range, pad=pad)  
        else:
            feats = []
            for i in range(N_views):
                feats.append(volume_feature[i](shape, rays_pts, pose_ref, depth_range=depth_range, pad=pad))
            feats_all = torch.stack(feats)
            ray_feats = feats_all.view(N_rays, N_samples, -1)
            del feats
        input_feat[..., :8*N_views] = ray_feats
        input_feat[..., 8*N_views:] = build_color_volume(rays_pts, pose_ref, imgs, img_feat, with_mask=True, downscale=img_downscale)
    else:
        input_feat = index_point_feature(shape, volume_feature, rays_ndc) if torch.is_tensor(volume_feature[0]) else volume_feature[0](rays_ndc, )
    return input_feat

def rendering(args, pose_ref, rays_pts, rays_ndc, depth_candidates, rays_o, rays_dir,
              volume_feature=None, imgs=None, network_fn=None, img_feat=None, depth_range=None, network_query_fn=None, white_bkgd=False, **kwargs):
    ...
    input_feat = gen_pts_feats(imgs, volume_feature, rays_pts, pose_ref, rays_ndc, args.feat_dim, \
                               img_feat, args.img_downscale, args.use_color_volume, args.net_type, depth_range=depth_range, pad=args.pad)
